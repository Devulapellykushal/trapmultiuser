...
class Geometry {
  ...
  subdivide(divisions = 1) {
    ...
    for (let div = 0; div < divisions; ++div) {
      ...
      for (const [ndx, face] of f.entries()) {
  ...
  get vertexData() {
    return new Float32Array(this.vertices.flatMap((v) => [...v.position]));
  }

  get normalData() {
    return new Float32Array(this.vertices.flatMap((v) => [...v.normal]));
  }

  get uvData() {
    return new Float32Array(this.vertices.flatMap((v) => [...v.uv]));
  }
...
function createProgram(gl, shaderSources, transformFeedbackVaryings, attribLocations) {
  ...
  for (const [ndx, type] of [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].entries()) {
...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    ...
    if (!this.discProgram) {
      throw new Error('[InfiniteMenu] Shader program failed to link; check console for compile logs.');
    }
    ...
    this.discVAO = makeVertexArray(
      ...
      [
        ...
        [
          makeBuffer(gl, this.discBuffers.normals, gl.STATIC_DRAW),
          this.discLocations.aModelNormal,
          3
        ],
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      if (!this.destroyed) {
        console.warn('[InfiniteMenu] Texture atlas failed', err);
      }
    });
  }
  ...
  #animate(deltaTime) {
    ...
    for (const [ndx, p] of positions.entries()) {
  ...
  destroy() {
    ...
    if (gl) {
      ...
      // Do not call WEBGL_lose_context here: React 18 StrictMode (or any remount) would
      // reuse this <canvas> and getContext('webgl2') often stays unusable until a full reload.
    }
  }
}
...
export default function InfiniteMenu({ items = [], scale = 1 }) {
  ...
  const effectiveItems = useMemo(() => (items.length > 0 ? items : defaultItems), [items]);
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index) => {
      ...
      if (n === 0) return;
      const itemIndex = index % n;
      setActiveItem(effectiveItems[itemIndex]);
    };
    ...
    if (canvas) {
      try {
        ...
        setActiveItem(effectiveItems[0] ?? null);
      } catch (error) {
        console.error('[InfiniteMenu]', error);
      }
    }
  ...
  const handleButtonClick = () => {
    ...
    if (activeItem.link.startsWith('http')) {
      ...
      globalThis.location.assign(activeItem.link);
    }
  };
