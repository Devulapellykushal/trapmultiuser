...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      // Never force minRadius larger than the viewport can show — huge radius = tiles off-screen ("no dome").
      const adaptiveMinRadius = Math.min(minRadius, Math.max(200, minDim * 0.72));
      radius = clamp(radius, adaptiveMinRadius, maxRadius);
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (originalImg) {
        ...
        animatingOverlay.append(img);
      }
      overlay.remove();
      rootRef.current.append(animatingOverlay);
      ...
      const cleanup = () => {
        /* eslint-disable sonarjs/no-nested-functions -- closing animation rAF chain */
          ...
          requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
              ...
              setTimeout(() => {
                ...
                el.style.opacity = '';
                openingRef.current = false;
                if (!draggingRef.current && rootRef.current?.dataset.enlarging !== 'true')
                  document.body.classList.remove('dg-scroll-lock');
              }, 300);
            });
          });
  ...
  const openItemFromElement = useCallback(
    (el) => {
      ...
      img.src = rawSrc;
      overlay.append(img);
      viewerRef.current.append(overlay);
